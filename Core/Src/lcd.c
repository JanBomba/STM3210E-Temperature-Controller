...
#define LCD_BASE        ((unsigned long)(0x60000000 | 0x0C000000))
...
{
  signed long  D;/* Decision Variable */ 
  unsigned long  CurX;/* Current X Value */
  unsigned long  CurY;/* Current Y Value */ 
...
void LCD_WriteBMP(unsigned long BmpAddress)
{
  ...
  /* Read bitmap size */
  size = *(volatile unsigned short *) (BmpAddress + 2);
  size |= (*(volatile unsigned short *) (BmpAddress + 4)) << 16;

  /* Get bitmap data address offset */
  index = *(volatile unsigned short *) (BmpAddress + 10);
  index |= (*(volatile unsigned short *) (BmpAddress + 12)) << 16;
  ...
  {
    LCD_WriteRAM(*(volatile unsigned short *)BmpAddress);
    BmpAddress += 2;
  }
...
//  /* Read bitmap size */
//  SPI_FLASH_BufferRead((unsigned char*)&size, BmpAddress + 2, 4);

//  /* get bitmap data address offset */
//  SPI_FLASH_BufferRead((unsigned char*)&i, BmpAddress + 10, 4);
//  
//  size = (size - i)/2;
